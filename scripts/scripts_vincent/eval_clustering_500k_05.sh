...
DATA_TEST="/gpfs/scratch/ehpc399/vincent/data/500k_mix/evaluation/evaluation_eCH/05/*.parquet"
CFG_DATA="config_files/config_hits_track_v4.yaml"
CFG_NET="src/models/wrapper/example_mode_gatr_noise.py"
MODEL_DIR="/gpfs/scratch/ehpc399/vincent/models/500k_05"
...
python -m src.train_lightning1 \
  --predict \
  --data-test "${DATA_TEST}" \
  --name-output "eval_clustering_500k_05.pkl" \
  ...
  --wandb-displayname "eval_clustering_500k_05" \
  --num-workers 16 \
  --gpus 0 \
  --batch-size 1 \
  --start-lr 1e-3 \
